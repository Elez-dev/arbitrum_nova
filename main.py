...
token_arr = [
    {'symbol': 'USDC',
     'address': Web3.toChecksumAddress('0x750ba8b76187092B0D1E87E28daaf484d1b5273b')},

    {'symbol': 'DAI',
     'address': Web3.toChecksumAddress('0xDA10009cBd5D07dd0CeCc66161FC93D7c9000da1')},

    {'symbol': 'WBTC',
     'address': Web3.toChecksumAddress('0x1d05e4e72cd994cdf976181cfb0707345763564d')},

    {'symbol': 'ARB',
     'address': Web3.toChecksumAddress('0xf823C3cD3CeBE0a1fA952ba88Dc9EEf8e0Bf46AD'),},
]
...
class Router:
    eth = Web3.toChecksumAddress('0x722e8bdd2ce80a4422e880164f2079488e115365')
    sushi_router = Web3.toChecksumAddress('0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506')
    rpc_router = Web3.toChecksumAddress('0x28e0f3ebab59a998c4f1019358388b5e2ca92cfa')
    arb_router = Web3.toChecksumAddress('0xee01c0cd76354c383b8c7b4e65ea88d00b06f36f')
    slingshot_router = Web3.toChecksumAddress('0xa135b6189d2e073dfbc33c30c86bb4ecea4e2ee5')
...
def swap_slingshot(private_key, token_to_buy, token_to_sold, amount, symbol):
    ...
    if token_to_sold == Router.eth:
        ...
        token_balance = Web3.fromWei(token_sold.functions.balanceOf(address_wallet).call(), 'ether')
    ...
    try:
        ...
        lock.acquire()
        print(f'\n>>> Swap {symbol} Slingshot| https://nova.arbiscan.io/tx/{web3.toHex(tx_hash)}', flush=True)
...
def swap_arb_buy(private_key, token_to_buy, amount, symbol):
    ...
    try:
        ...
        tx_hash = web3.eth.send_raw_transaction(signed_txn.rawTransaction)
        lock.acquire()
        print(f'\n>>> Buy {symbol} ArbSwap| https://nova.arbiscan.io/tx/{web3.toHex(tx_hash)}', flush=True)
        print(f'    {address_wallet}', flush=True)
        lock.release()
    except Exception as error:
        lock.acquire()
...
def swap_arb_sold(private_key, token_to_sold, symbol):
    ...
    try:
        ...
        tx_hash = web3.eth.send_raw_transaction(signed_txn.rawTransaction)
        lock.acquire()
        print(f'\n>>> Sold {symbol} ArbSwap | https://nova.arbiscan.io/tx/{web3.toHex(tx_hash)}', flush=True)
        print(f'    {address_wallet}', flush=True)
        lock.release()
    except Exception as error:
        lock.acquire()
...
def swap_rpc_buy(private_key, token_to_buy, amount, symbol):
    ...
    try:
        ...
        tx_hash = web3.eth.send_raw_transaction(signed_txn.rawTransaction)
        lock.acquire()
        print(f'\n>>> Buy {symbol} RPC| https://nova.arbiscan.io/tx/{web3.toHex(tx_hash)}', flush=True)
        print(f'    {address_wallet}', flush=True)
        lock.release()
    except Exception as error:
        lock.acquire()
...
def swap_rpc_sold(private_key, token_to_sold, symbol):
    ...
    try:
        ...
        tx_hash = web3.eth.send_raw_transaction(signed_txn.rawTransaction)
        lock.acquire()
        print(f'\n>>> Sold {symbol} RPC| https://nova.arbiscan.io/tx/{web3.toHex(tx_hash)}', flush=True)
        print(f'    {address_wallet}', flush=True)
        lock.release()
    except Exception as error:
        lock.acquire()
...
def swap_sushi_buy(private_key, token_to_buy, amount, symbol):
    ...
    try:
        ...
        tx_hash = web3.eth.send_raw_transaction(signed_txn.rawTransaction)
        lock.acquire()
        print(f'\n>>> Buy {symbol} Sushiswap| https://nova.arbiscan.io/tx/{web3.toHex(tx_hash)}', flush=True)
        print(f'    {address_wallet}', flush=True)
        lock.release()
    except Exception as error:
        lock.acquire()
...
def swap_sushi_sold(private_key, token_to_sold, symbol):
    ...
    try:
        ...
        tx_hash = web3.eth.send_raw_transaction(signed_txn.rawTransaction)
        lock.acquire()
        print(f'\n>>> Sold {symbol} Sushiswap| https://nova.arbiscan.io/tx/{web3.toHex(tx_hash)}', flush=True)
        print(f'    {address_wallet}', flush=True)
        lock.release()
    except Exception as error:
        lock.acquire()
...
def approve(private_key, token_to_approve, address_to_approve, symbol):
    ...
    try:
        tx = token_contract.functions.approve(address_to_approve, max_amount).buildTransaction({
            'from': address_wallet,
            'gasPrice': web3.eth.gas_price,
            'gas': gas,
            'nonce': web3.eth.getTransactionCount(address_wallet)
        ...
        lock.acquire()
        print(f'\n>>> {symbol} approve : https://nova.arbiscan.io/tx/{web3.toHex(tx_hash)}')
        print(f'    {address_wallet}', flush=True)
        lock.release()
    except Exception as error:
        lock.acquire()
...
def check_allowance(private_key, token_to_buy, router):
    ...
    allowance = Web3.fromWei(token_contract.functions.allowance(address_wallet, router).call(), 'ether')
    return allowance
...
def add_liquidity(private_key, token_to_buy, amount, symbol, router):
    ...
    try:
        ...
        if allowance < 10:
            ...
            time.sleep(25)
        amount_ = Web3.toWei(amount, 'ether')
        ...
        tx_hash = web3.eth.send_raw_transaction(signed_txn.rawTransaction)
        lock.acquire()
        print(f'\n>>> add liquidity {symbol} | https://nova.arbiscan.io/tx/{web3.toHex(tx_hash)}', flush=True)
        print(f'    {address_wallet}', flush=True)
        lock.release()
    except Exception as error:
        lock.acquire()
